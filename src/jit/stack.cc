...
TR::IlValue* CheckResult(TR::IlValue* v) {
...
void VirtualStack::MergeInto(TR::IlBuilder* b, const VirtualStack* other) {
  TR_ASSERT(values_.size() == other->values_.size(), "Attempt to merge divergent stacks");
  ...
  while (this_it != values_.end()) {
    if (*this_it != *other_it) {
      TR_ASSERT(*this_it && *other_it, "Attempt to merge divergent stacks");
      TR_ASSERT((*this_it)->getDataType() == (*other_it)->getDataType(),
                "Attempt to merge divergent stacks");
...
void VirtualStack::CopyTo(VirtualStack* other) {
